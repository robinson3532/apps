...
class UndoableEntry(tk.Text):
    ...
    def undo(self, event=None):
        if self.undo_stack:
            text = self.undo_stack.pop().rstrip('\n')  # 末尾改行を除去
            self.redo_stack.append(self.get("1.0", "end-1c"))  # 現在の状態も改行除去
            self.delete("1.0", "end")
            self.insert("1.0", text)

    def redo(self, event=None):
        if self.redo_stack:
            text = self.redo_stack.pop().rstrip('\n')
    ...
    def on_key_press(self, event):
        if event.keysym not in ["Control_L", "Control_R", "z", "y"]:
            current_text = self.get("1.0", "end-1c").rstrip('\n')  # 改行除去して保存
...
class MemoApp:
    ...
    def setup_ui(self):
        ...
        # self.content_text = scrolledtext.ScrolledText(content_frame, font=("Arial", 11), wrap=tk.WORD, undo=True)
        self.content_text = UndoableEntry(content_frame, font=("Arial", 11), wrap=tk.WORD)
